...
文件名 & 功能描述 \\
\midrule
\texttt{paper/code/q1\_solution.py} & 问题1 全流程脚本：多维特征体系 / 退化规律可视化 / 三阶段划分 / 因子量化（随机森林+灰色关联+熵权+组合权重）/ 综合退化模型标定 / 合理性验证，输出 \texttt{q1\_results.json} \\
\texttt{paper/code/q2\_solution.py} & 问题2 全流程脚本：SOH 辨识与置信区间 / 多模型 RUL 预测对比 / EEMD-GRU 主模型 / 工况扰动 / 灵敏度分析，输出 \texttt{q2\_results.json} \\
\texttt{paper/code/analyze.py} & 共享数据分析脚本：问题1 退化曲线、内阻、三阶段、因子重要性图；问题2 模型对比图，输出 \texttt{results.json} \\
\texttt{paper/code/preprocess\_figs.py} & 数据预处理图表脚本：\texttt{q0\_preprocess}、\texttt{q1\_resistance} \\
\texttt{data/dataset/generate\_sim.py} & 仿真数据生成器（成员A）：多因子退化模型 + 容量再生 + AR(1) 噪声 + 批次差异 \\
\texttt{data/dataset/generate\_data.py} & 派生数据表生成器：健康指标表 + 选中结果表 \\
\texttt{data/dataset/convert\_to\_spec.py} & 数据格式规范 v1.1 转换脚本 \\
\bottomrule
\end{tabularx}
\label{tab:files}
\end{table}
...
文件 & 说明 \\
\midrule
\texttt{solution\_A/问题1--问题4} & 各问题分步骤解答文档（Markdown，含建模思路与结果讨论） \\
\texttt{paper/figures/*.png} & 论文全部图件：\texttt{q0\_preprocess}、\texttt{q1\_*}（4 张）、\texttt{q2\_*}（7 张）、\texttt{q3\_grade}、\texttt{q4\_sens} 等 \\
\texttt{data/数据格式规范1.1.md} & 全队统一数据格式规范（字段、命名、接口） \\
\texttt{references/} & 备赛资料库与知识库，索引见附录 D \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{verbatim}
# 1) 数据生成（成员A，数据已生成则跳过）
python data/dataset/generate_sim.py
python data/dataset/generate_data.py

# 2) 求解与绘图（输出 paper/figures/*.png 与 paper/code/*.json）
python paper/code/q1_solution.py
python paper/code/q2_solution.py
python paper/code/analyze.py
python paper/code/preprocess_figs.py
\end{verbatim}
...
\lstinputlisting[language=Python, caption=问题1 全流程求解脚本]{code/q1_solution.py}

\subsection{问题2 求解脚本（\texttt{q2\_solution.py}）}
\lstinputlisting[language=Python, caption=问题2 求解脚本（EEMD-GRU 主模型）]{code/q2_solution.py}

\subsection{共享数据分析脚本（\texttt{analyze.py}）}
\lstinputlisting[language=Python, caption=共享数据分析脚本]{code/analyze.py}

\subsection{数据预处理图表脚本（\texttt{preprocess\_figs.py}）}
\lstinputlisting[language=Python, caption=数据预处理图表脚本]{code/preprocess_figs.py}

\subsection{仿真数据生成器（\texttt{generate\_sim.py}）}
\lstinputlisting[language=Python, caption=仿真数据生成器]{../data/dataset/generate_sim.py}

\subsection{派生数据表生成器（\texttt{generate\_data.py}）}
\lstinputlisting[language=Python, caption=派生数据表生成器]{../data/dataset/generate_data.py}

\subsection{数据格式规范转换脚本（\texttt{convert\_to\_spec.py}）}
\lstinputlisting[language=Python, caption=数据格式规范转换脚本]{../data/dataset/convert_to_spec.py}
...
\subsection{知识库资料索引}
支撑材料中 \texttt{references/} 为备赛资料库与知识库，目录结构与关键内容如下（完整索引见 \texttt{references/README\_资料索引.md}）：
\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{LL}
